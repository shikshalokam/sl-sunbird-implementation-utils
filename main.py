        ...
        if sheetEnv.strip().lower() == 'details':
            ...
            for row_index_env in range(2, detailsEnvSheet.nrows):
                dictDetailsEnv = {keysEnv[col_index_env]: detailsEnvSheet.cell(row_index_env, col_index_env).value
                                    for
                                    col_index_env in range(detailsEnvSheet.ncols)}
                programNameInp = dictDetailsEnv['solution_name'].encode('utf-8').decode('utf-8') if dictDetailsEnv['solution_name'] else terminatingMessage("\"Title of the Program\" must not be Empty in \"Program details\" sheet")
...
def getProgramInfo(accessTokenUser, solutionName_for_folder_path, programNameInp):
    ...
    programUrl = config.get(environment, 'INTERNAL_KONG_IP_CORE') + config.get(environment, 'fetchProgramInfoApiUrl') + programNameInp.lstrip().rstrip()
    terminatingMessage
...
def fetchUserDetails(environment, accessToken, dikshaId):
    global OrgName
    url = "http://11.3.16.225/userorg/private/user/" + config.get(environment, 'userInfoApiUrl')
...
def fetchScopeRole(solutionName_for_folder_path, accessToken, roleNameList):
    urlFetchRolesListApi = config.get(environment, 'INTERNAL_KONG_IP_SURVEY') + config.get(environment, 'listOfRolesApi')
                ...
                if sheetEnv.strip().lower() == 'questions':
                    ...
                    for row_index_env in range(2, detailsEnvSheet.nrows):
                        ...
                        if not dictDetailsEnv['question_weightage']:
                            print(dictDetailsEnv['question_weightage'])
            ...
            if sheetEnv == "Instructions":
                ...
                if sheetEnv.strip().lower() == 'details':
                    ...
                    for row_index_env in range(2, detailsEnvSheet.nrows):
                        ...
                        dikshaLoginId = dictDetailsEnv['creator_username'].encode('utf-8').decode('utf-8') if dictDetailsEnv['creator_username'] else terminatingMessage("\"Diksha_loginId\" must not be Empty in \"details\" sheet")
                        # creator = dictDetailsEnv['Name_of_the_creator'] if dictDetailsEnv['Name_of_the_creator'] else terminatingMessage("\"Name_of_the_creator\" must not be Empty in \"details\" sheet")
                        ccUserDetails = fetchUserDetails(environment, accessToken, dikshaLoginId)
                        if not "CONTENT_CREATOR" in ccUserDetails[3]:
                            terminatingMessage("---> "+dikshaLoginId +" is not a CONTENT_CREATOR in Diksha " + environment)
...
def fetchSolutionDetailsFromProgramSheet(solutionName_for_folder_path, programFile, solutionId, accessToken):
    ...
    # print(responseFetchSolutionApi.text)
    ...
    if responseFetchSolutionApi.status_code == 200:
        ...
        xfile = xlrd.open_workbook(programFile, on_demand=True)
            ...
            if responseQuestionUploadApi.status_code == 200:
                ...
                if responseImportSoluTemplateApi.status_code == 200:
                    ...
                    if responseSurveyProgramMappingApi.status_code == 200:
                        ...
                        # solutionDetails = fetchSolutionDetailsFromProgramSheet(parentFolder, solutionIdSuc,
                        #                                                        accessToken)
...
def checkEntityOfSolution(projectName_for_folder_path, solutionNameOrId, accessToken):
    ...
    searchSolutionurl = config.get(environment, 'INTERNAL_KONG_IP_CORE') + config.get(environment,'fetchSolutionDetails') + "observation&page=1&limit=100&search=" + solutionNameOrId
    ...
    if searchSolutionresponse.status_code == 200:
        ...
        for listOfSoulution in range(0, len(searchSolutionjson["result"]["data"])):
            ...
            solutionDetailsurl = config.get(environment, 'INTERNAL_KONG_IP_CORE') + config.get(environment, 'fetchSolutionDoc') + solution_id
...
def projectUpload(projectFile, projectName_for_folder_path, accessToken):
    urlProjectUploadApi = config.get(environment, 'INTERNAL_KONG_IP_PROJECT') + config.get(environment, 'projectUploadApi')
...
def taskUpload(projectFile, projectName_for_folder_path, accessToken):
    ...
    for projectInternal in projectInternalfile:
        ...
        if str(project_id).strip() == "Could not pushed to kafka":
            fetchProjectIdApi = config.get(environment, 'INTERNAL_KONG_IP_PROJECT') + config.get(environment, 'FetchProjectList')
        ...
        urlTasksUploadApi = config.get(environment, 'INTERNAL_KONG_IP_PROJECT') + config.get(environment, 'taskUploadApi') + project_id
...
def fetchCertificateBaseTemplate(filePathAddProject,accessToken,projectName_for_folder_path):
    ...
    urldbFind = config.get(environment, 'INTERNAL_KONG_IP_CORE') + config.get(environment, 'dbfindapi')
    headerdbFindApi = {
        'Authorization': config.get(environment, 'Authorization'),
        'X-authenticated-user-token': accessToken,
        'X-Channel-id': config.get(environment, 'X-Channel-id'),
...
def prepareaddingcertificatetemp(filePathAddProject, projectName_for_folder_path, accessToken, solutionId, programID,baseTemplate_id):
    ...
    urladdcertificate = config.get(environment, 'INTERNAL_KONG_IP_CORE') + config.get(environment, 'Addcertificatetemplate')
    ...
    urluploadcertificatepi = config.get(environment, 'INTERNAL_KONG_IP_CORE')+config.get(environment, 'uploadcertificatetosvg') + certificatetemplateid
    ...
    if responseDownloadsvgApi.status_code == 200:
        ...
        urlsolutionupdateapi = config.get(environment, 'INTERNAL_KONG_IP_CORE')+config.get(environment, 'updatecertificatesolu') + str(solutionId)
        ...
        urlprojecttemplateapi = config.get(environment, 'INTERNAL_KONG_IP_PROJECT')+config.get(environment, 'updateprojecttemplate') + projectTemplateId
    ...
    for prosheet in projectsheetforcertificate:
        if prosheet.strip().lower() == 'Certificate details'.lower():
            ...
            for row_index_env in range(2, detailsEnvSheet.nrows):
                ...
                urleditnigsvgApi = config.get(environment, 'INTERNAL_KONG_IP_CORE') + config.get(environment, 'editsvgtemp') + baseTemplateId
                headereditingsvgApi = {
                    'Authorization': config.get(environment, 'Authorization'),
                    'X-authenticated-user-token': accessToken,
                    'X-Channel-id': config.get(environment, 'X-Channel-id'),
...
def solutionCreationAndMapping(projectName_for_folder_path, entityToUpload, listOfFoundRoles, accessToken):
    ...
    for projectInternal in projectInternalfile:
        ...
        urlCreateProjectSolutionApi = config.get(environment, 'INTERNAL_KONG_IP_PROJECT') + config.get(environment, 'projectSolutionCreationApi')
        headerCreateSolutionApi = {
            'Content-Type': config.get(environment, 'Content-Type'),
            'Authorization': config.get(environment, 'Authorization'),
            'X-authenticated-user-token': accessToken,
        ...
        if responseCreateSolutionApi.status_code == 200:
            ...
            urlMapProjectSolutionApi = config.get(environment, 'INTERNAL_KONG_IP_PROJECT') + config.get(environment, 'mapSolutionToProject')
...
def mainFunc(MainFilePath, addObservationSolution, millisecond, scopeEntityType=scopeEntityType):
    global isProgramnamePresent
    print(addObservationSolution)
    scopeEntityType = scopeEntityType
    print("reached here")
   
    parentFolder = createFileStruct(MainFilePath, addObservationSolution)
    accessToken = generateAccessToken(parentFolder)
    # accessToken = "eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJHUnh4OHVyNDNwWEgzX1FNekJXZXJRUFdyWDAyUEprSzlDemwzaGM2MGZBIn0..DLYhuMfTucfFuuVhnTkpgx61_Uc5lNWNATmVmWoU5jtuH8MCehOoVvMv35Ng9--_Y_ips-5JWjH3i-O23NJNc-yIJgivdFThzqPbVoLPoC12IjMD8UGEZ8Lu2n0zV8-xqDh7CZgZCj6OGETJ-qwF6oqxgvGZxKzttbf6gwjkEaFG3GaCJ6M40HYfVomHKUboTK7tXPIV1G_m0tKTO8XHilqeEo3cDpsBXgEI4V2ZfEgIz7JXR-0U7lVflynUwjnTObAX0DZUi0bEuzlZmyClsmPwdpDWS6BKtZmoENlpLabwtn1XOzntmRrneya4m5iDWd2NMUrr7KEflV0cfP3ChQ"

    programsFileCheck(addObservationSolution, accessToken, parentFolder, MainFilePath)
    ...
    if typeofSolution == 1 or typeofSolution == 5:
        ...
        solutionUpdate(parentFolder, accessToken, solutionId, bodySolutionUpdate)
        # need to remove 
        # solutionDetails = fetchSolutionDetailsFromProgramSheet(parentFolder, addObservationSolution, solutionId, accessToken)
        ...
        if isProgramnamePresent:
            ...
            if childId[0]:
                ...
                messageArr = ["URL : " + str(solutionDetailsurl),
                            "Status Code : " + str(solutionDetailsresponse["result"]),
                            ]
                # need to remove
                # prepareProgramSuccessSheet(MainFilePath, parentFolder, programFile, childId[1], childId[0],
                                            # accessToken)
...
parser.add_argument('--resourceFile', '--programFile', type=valid_file)
...
print(resourceLinkOrExtPGMcopy)
...
if not os.path.isdir('InputFiles'):
    ...
    # Copy the local Excel file to the destination directory
